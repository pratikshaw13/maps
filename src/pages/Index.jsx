...
const Index = () => {
  const [households, setHouseholds] = useState(generateHouseholds());
  const [alerts, setAlerts] = useState(generateAlerts());
  const [additionalTransformers, setAdditionalTransformers] = useState([]);
  ...
  const handlePhaseChange = (householdId, newPhase) => {
    setHouseholds(prev => prev.map(h => 
      h.id === householdId 
        ? { ...h, phase: newPhase, status: 'healthy' }
        : h
    ));
